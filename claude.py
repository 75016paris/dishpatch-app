...
sub_df = add_ended_at_utc(sub_df)
...
sub_df = calculate_duration(sub_df)
...
def customer_df(df):
    ...
    cus_df = df[~df['is_gifted_member']].groupby('customer_id').agg({
        'real_duration': 'last',
        'trial_duration': 'last',
        'trial_start_utc': 'first',
        'canceled_at_utc': 'last',
        'is_full_member': 'last',
        'canceled_during_trial': 'last',
        'canceled_during_refund_period': 'last',
        'status': 'last',
        'created_utc': 'min',
        'refund_period_end_utc': 'last',
        'current_period_start_utc': 'last',
        'current_period_end_utc': 'last',
        }).reset_index()

    days_since_trial = (today_date - cus_df['trial_start_utc']).dt.days
    cus_df['paid_days'] = np.maximum(0, days_since_trial - cus_df['trial_duration'])

    return cus_df   

cus_df = customer_df(df)
...
def cus_renewal(cus_df):
    """Calculate renewal rates with correct logic"""
    df = cus_df.copy()

    # Number of Customers who had Trial
    trial_customers = df[df['trial_start_utc'].notna()]
    print(f"Customers who had trial: {len(trial_customers)}")

    # Number of Customers who became Full Members (from trial)
    trial_to_full_customers = df[
        (df['trial_start_utc'].notna()) & 
        (df['is_full_member'])
    ]
    print(f"Trial customers who became full members: {len(trial_to_full_customers)}")

    # Trial > Full Member Conversion Rate
    conversion_rate = (len(trial_to_full_customers) / len(trial_customers) * 100) if len(trial_customers) > 0 else 0
    print(f"Conversion rate: {conversion_rate:.2f}%")

    print('------------------------')

    # PREMIÈRE ANNÉE - Customers actuellement en 1ère année
    first_year_customers = df[
        (df['paid_days'] <= 366) &
        (df['is_full_member'])
    ]
    print(f"Customers currently in first year: {len(first_year_customers)}")

    # Cancellations pendant la 1ère année
    canceled_during_first_year = first_year_customers[first_year_customers['canceled_at_utc'].notna()]
    print(f"Canceled during first year: {len(canceled_during_first_year)}")

    first_year_cancellation_rate = (len(canceled_during_first_year) / len(first_year_customers) * 100) if len(first_year_customers) > 0 else 0
    print(f"First year cancellation rate: {first_year_cancellation_rate:.2f}%")

    print('------------------------')

    # RENEWAL RATE YEAR 1 → YEAR 2
    # Customers éligibles = ceux qui ont atteint 365 jours payants (ont terminé leur 1ère année)
    eligible_for_year2 = df[df['paid_days'] >= 365]
    print(f"Customers eligible for Year 2 renewal: {len(eligible_for_year2)}")

    # Customers qui ont effectivement renouvelé = ceux qui ont dépassé 365 jours
    renewed_to_year2 = df[
    (df['paid_days'] > 365) & 
    (df['is_full_member'])  # ← AJOUT IMPORTANT
]
    print(f"Customers who renewed to Year 2: {len(renewed_to_year2)}")

    # VRAI Renewal Rate Year 1 → Year 2
    renewal_rate_y1_to_y2 = (len(renewed_to_year2) / len(eligible_for_year2) * 100) if len(eligible_for_year2) > 0 else 0
    print(f"Renewal Rate Y1→Y2: {renewal_rate_y1_to_y2:.2f}%")

    # Customers actuellement en 2ème année (365 < paid_days ≤ 730)
    currently_in_year2 = df[
        (df['paid_days'] > 365) & 
        (df['paid_days'] <= 730)
    ]
    print(f"Customers currently in Year 2: {len(currently_in_year2)}")


    churned_during_year2 = currently_in_year2[ 
        (currently_in_year2['canceled_at_utc'].notna())
    ]
    print(f"Customers who churned during Year 2: {len(churned_during_year2)}")

    print('------------------------')

    # RENEWAL RATE YEAR 2 → YEAR 3
    # Customers éligibles = ceux qui ont atteint 730 jours payants (ont terminé leur 2ème année)
    eligible_for_year3 = df[df['paid_days'] >= 730]
    print(f"Customers eligible for Year 3 renewal: {len(eligible_for_year3)}")

    # Customers qui ont effectivement renouvelé = ceux qui ont dépassé 730 jours
    renewed_to_year3 = df[(df['paid_days'] > 730) &
                          (df['is_full_member'])]
    print(f"Customers who renewed to Year 3: {len(renewed_to_year3)}")

    # VRAI Renewal Rate Year 2 → Year 3
    renewal_rate_y2_to_y3 = (len(renewed_to_year3) / len(eligible_for_year3) * 100) if len(eligible_for_year3) > 0 else 0
    print(f"Renewal Rate Y2→Y3: {renewal_rate_y2_to_y3:.2f}%")

    # Customers actuellement en 3ème année
    currently_in_year3 = df[df['paid_days'] > 730]
    print(f"Customers currently in Year 3+: {len(currently_in_year3)}")

    churned_during_year3 = currently_in_year3[
        (currently_in_year3['canceled_at_utc'].notna())]

    print('------------------------')

    active_first_year = first_year_customers[first_year_customers['status'] == 'active']
    active_year2 = currently_in_year2[currently_in_year2['status'] == 'active']
    active_year3 = currently_in_year3[currently_in_year3['status'] == 'active']
    print(f"Active customers in Year 1: {len(active_first_year)}, Year 2: {len(active_year2)}, Year 3+: {len(active_year3)}")

    # CHURN ANALYSIS
    # Customers qui ont churné pendant leur 2ème année


    print('========================')
    print("📊 SUMMARY:")
    print(f"Conversion Rate (Trial→Full): {conversion_rate:.1f}%")
    print(f"Renewal Rate Y1→Y2: {renewal_rate_y1_to_y2:.1f}%")
    print(f"Renewal Rate Y2→Y3: {renewal_rate_y2_to_y3:.1f}%")

    # Return corrected metrics
    renewal_dict = {
        'trial_customers': len(trial_customers),
        'trial_to_full_customers': len(trial_to_full_customers),
        'conversion_rate': round(conversion_rate, 2),
        
        'first_year_customers': len(first_year_customers),
        ...
        'eligible_for_year2': len(eligible_for_year2),
        'renewed_to_year2': len(renewed_to_year2),
        'renewal_rate_y1_to_y2': round(renewal_rate_y1_to_y2, 2),
        'currently_in_year2': len(currently_in_year2),
        
        'eligible_for_year3': len(eligible_for_year3),
        'renewed_to_year3': len(renewed_to_year3),
        'renewal_rate_y2_to_y3': round(renewal_rate_y2_to_y3, 2),
        'currently_in_year3': len(currently_in_year3),
       
        'active_first_year': len(active_first_year),
        'active_year2': len(active_year2),
        'active_year3': len(active_year3),

        'trial_customers_df': trial_customers,
        'first_year_customers_df': first_year_customers,
        'currently_in_year2_df': currently_in_year2,
        'currently_in_year3_df': currently_in_year3,
        'eligible_for_year2_df': eligible_for_year2,
        'renewed_to_year2_df': renewed_to_year2,
        'renewed_to_year3_df': renewed_to_year3,

        'churned_during_year2': len(churned_during_year2),
        'churned_during_year3': len(churned_during_year3)
    }
...
y2_df = renewal_dict['currently_in_year2_df']
#y2_df[y2_df['current_period_start_utc'] == '2023-10-10 08:59:00+0000']
#df[df['customer_id'] == 'cus_OjRuBDQ8BaX2bp']     
...
def weekly_flow_all_time(cus_df, today_date):
    ...
    trial_customers_df = renewal_dict['trial_customers_df']
    first_year_customers_df = renewal_dict['first_year_customers_df']
    currently_in_year2_df = renewal_dict['currently_in_year2_df']
    currently_in_year3_df = renewal_dict['currently_in_year3_df']
    renewed_to_year2_df = renewal_dict['renewed_to_year2_df']
    renewed_to_year3_df = renewal_dict['renewed_to_year3_df']
    ...
    conversion_customers = cus_df[
        cus_df['is_full_member'] == True
    ]

    weekly_conversions = conversion_customers.groupby(
        pd.Grouper(key='created_utc', freq='W-MON')).size()

    y1_to_y2_customers = currently_in_year2_df.copy()
    ...
    # === CUMULATIVE LINE PLOT ===
    ax_twin = ax.twinx()
    ax_twin.plot(x_pos, net_weekly, color='darkblue', linewidth=1, \
    ...
    immature_cutoff = today_date - pd.Timedelta(days=50)  # Extended for customers
...
weekly_flow_all_time(cus_df, today_date)
...
#         # === CONVERSIONS ===
#         week_conversions = df[
#             (cus_df['created_utc'] >= week_info['week_start']) &
#             (cus_df['created_utc'] <= week_info['week_end']) &
#             (cus_df['trial_start_utc'].notna()) &  # Had a trial
#             (cus_df['is_full_member'] == True) &   # Became full member
#             (cus_df['refund_period_end_utc'] < today_date)]
...
#
#
# metrics_all = weekly_flow_all_time(df=cus_df, today_date=today_date)
#
#
# # %%
# #######################################################################################
...
# #     - Sans trial = Gifted OU Renouvellements automatiques (même subscription_id)
# #     """
# #
...
# #             for i, (date, subscription_id) in enumerate(zip(dates, sub_ids)):
# #                 print(f"         {i+1}. {date} - {subscription_id[:15]}...")
# #
...
# #     print(f"   Renouvellements = même customer, nouvelles subscription_id")
# #
