...
if uploaded_file:
    ...
    st.pyplot(fig_cohort)
        ...
        col1, col2, col3 = st.columns(3)
    col1.metric("Drop-off during trial:", f"{renewal_flow_results['avg_renewals_per_week']:.1f}")
    col2.metric("Drop-off during refund:", f"{renewal_flow_results['avg_post_churn_per_week']:.1f}")
    col2.metric("Total drop-off:", f"{renewal_flow_results['avg_refund_per_week']:.1f}")
