...
if uploaded_file:
    ...
    col1, col2, col3 = st.columns(3)
    col1.metric("Average renewals per week (all time):", f"{weekly_flow_all_time_result['avg_conversions_per_week']:.1f}")
    col2.metric("Average Post-Renewal per week (all time):", f"{weekly_flow_all_time_result['avg_net_per_week']:.1f}")
    col2.metric("Average Refund per week (all time):", f"{renewal_flow_results['avg_post_churn_per_week']:.1f}")
