...
st.markdown("""
<style>
/* Plusieurs sélecteurs pour plus de compatibilité */
...
if uploaded_file:
    ...
    col1.markdown(f"""
    **Max week:** {trials_metrics_8w['max_week']} <small>({trials_metrics_8w['max_week_label']})</small>
    """, unsafe_allow_html=True)

    col2.markdown(f"""
    **Min week:** {trials_metrics_8w['min_week']} <small>({trials_metrics_8w['min_week_label']})</small>
    """, unsafe_allow_html=True)
