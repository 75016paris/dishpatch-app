...
if uploaded_file:
    ...
    col1, col2, col3 = st.columns(3)
    col1.metric("Last week :", trials_metrics_8w['latest_week'])
    col2.metric("Previous week :", trials_metrics_8w['previous_week'])
    col3.metric(
        "Week-over-week change :",
        f"{trials_metrics_8w['week_over_week_change']} ({trials_metrics_8w['week_over_week_pct']:.1f}%)",
        delta_color="normal" if trials_metrics_8w['week_over_week_change'] > 0 else "inverse"
    )
